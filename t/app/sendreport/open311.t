...
test_overrides fixmystreet =>
    ...
    superhashof({
        handler => isa('FixMyStreet::Cobrand::Cambridgeshire'),
        'open311' => noclass(superhashof(\%standard_open311_parameters)),
        problem_user => noclass(superhashof({
            name => re('^\d+ Fred Bloggs'),
        })),
        discard_changes => 1,
    });

test_overrides fixmybarangay =>
    {
        body_name => 'FixMyBarangay',
        body_id   => 1000, # dummy
    },
    superhashof({
        handler => isa('FixMyStreet::Cobrand::FixMyBarangay'),
        'open311' => noclass(superhashof(\%standard_open311_parameters)),
        discard_changes => 1,
        problem_extra => [
            { name => 'external_id', value => re('[0-9]+') },
...
sub test_overrides {
    ...
    subtest "$cobrand ($input->{body_name}) overrides" => sub {
        ...
        FixMyStreet::override_config {
            ...
            my $row = FixMyStreet::App->model('DB::Problem')->create( {
                title => 'Problem',
...
BEGIN {
    ...
    around send_service_request => sub {
        ...
        $data{extra} = $extra;
        $data{problem_extra} = $problem->extra;
